...
public class MenuSizeValidatorForMealSave implements ConstraintValidator<MenuSize, List<MealFoodParam>> {

    @Override
    public boolean isValid(List<MealFoodParam> menus, ConstraintValidatorContext context) {
        return (!menus.isEmpty()) && (menus.size() <= MENU_MAX_SIZE_PER_MEAL);
    }
}
